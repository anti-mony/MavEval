...
class Controller:
    ...
    def add(self, recipie, index='recipies'):
        """
        ...
        """
        try:
            validate(instance=recipie, schema=self._schema)
            response = self._db.index(index=index, body=recipie)
            return response
        except ValidationError as e:
            return (
                "Validation Error, there was an error in the object received",
                400)
        except Exception as e:
            print(e, file=stderr)
            return ("Internal Server Error", 500)

    def get(self, id=None, index='recipies', servings=None):
        """
